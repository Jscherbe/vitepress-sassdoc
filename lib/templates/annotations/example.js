...
import { codeGroupBlock, codeGroup, preview } from "../../template-helpers.js";
...
export default ({ item, options, headlineLevel }) => {
  ...
  if (example) {
    const $examples = example.map(({ 
      ...
      if (compiled) {
        let $groupBlocks = [ 
          codeGroupBlock(code, type, type.toUpperCase()),
          codeGroupBlock(compiled, "css", "CSS")
        ];
        exampleMarkup = codeGroup(joinMarkup($groupBlocks));
      } else {
